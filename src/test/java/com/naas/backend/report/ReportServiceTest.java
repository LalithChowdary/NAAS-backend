...
class ReportServiceTest {
    ...
    void getMonthlySummary_shouldReturnCorrectAggregates() {
        ...
        when(jdbcTemplate.queryForObject(contains("SUM(total_amount) FROM bills"), eq(BigDecimal.class), anyString()))
                .thenReturn(new BigDecimal("15000.00"));

        when(jdbcTemplate.queryForObject(contains("SUM(amount) FROM payments"), eq(BigDecimal.class), any(), any()))
                .thenReturn(new BigDecimal("12000.00"));

        when(jdbcTemplate.queryForObject(contains("COUNT(*) FROM delivery_records WHERE delivery_date"), eq(Long.class), any(LocalDate.class), any(LocalDate.class)))
    ...
    void getMonthlySummary_whenNoBillingData_shouldReturnZeroDefaults() {
        ...
        // Assert — nulls must be mapped to safe zero values (not throw NullPointerException)
    ...
    void getMonthlySummary_shouldParseMonthStringToCalculateDateRange() {
        // Arrange
        when(jdbcTemplate.queryForObject(contains("COUNT(*) FROM subscriptions"), eq(Long.class))).thenReturn(5L);
        when(jdbcTemplate.queryForObject(contains("SUM(total_amount)"), eq(BigDecimal.class), anyString())).thenReturn(null);
        when(jdbcTemplate.queryForObject(contains("SUM(amount)"), eq(BigDecimal.class), any(), any())).thenReturn(null);
        when(jdbcTemplate.queryForObject(contains("COUNT(*) FROM delivery_records"), eq(Long.class), any(LocalDate.class), any(LocalDate.class)))
    ...
    void getDeliverySummary_shouldReturnDailyBreakdown() {
        ...
        List<DeliverySummaryResponse> mockSummary = List.of(
                DeliverySummaryResponse.builder().date(start).totalAssigned(100L).delivered(90L).cancelled(5L).pending(5L).build(),
                DeliverySummaryResponse.builder().date(start.plusDays(1)).totalAssigned(95L).delivered(85L).cancelled(3L).pending(7L).build(),
                DeliverySummaryResponse.builder().date(start.plusDays(2)).totalAssigned(105L).delivered(100L).cancelled(2L).pending(3L).build()
        );

        when(jdbcTemplate.query(anyString(), ArgumentMatchers.<RowMapper<DeliverySummaryResponse>>any(), any(LocalDate.class), any(LocalDate.class)))
    ...
    void getDeliveryPersonnelPayment_shouldCalculateCommissionCorrectly() {
        ...
        List<DeliveryPersonnelPaymentResponse> mockData = List.of(
                DeliveryPersonnelPaymentResponse.builder()
                        ...
                        .totalDeliveryValue(new BigDecimal("10000.00"))
                        .paymentAmount(new BigDecimal("250.00")) // 2.5% of 10000
                        .build()
        );

        when(jdbcTemplate.query(anyString(), ArgumentMatchers.<RowMapper<DeliveryPersonnelPaymentResponse>>any(), any(LocalDate.class), any(LocalDate.class)))
                .thenReturn(mockData);
    ...
    void getWhoReceivedWhat_shouldReturnDetailedDeliveryLog() {
        ...
        when(jdbcTemplate.query(anyString(), ArgumentMatchers.<RowMapper<WhoReceivedWhatResponse>>any(), any(LocalDate.class), any(LocalDate.class)))
                .thenReturn(mockLog);
    ...
    void getWhoReceivedWhat_shouldIncludeAllDeliveryStatuses() {
        ...
        // Assert — both statuses present; report covers ALL deliveries not just successful ones
        assertThat(result).hasSize(2);
